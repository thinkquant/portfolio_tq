...
export async function listProjects(firestore: Firestore | null): Promise<ProjectRecord[]> {
...
export async function listRuns(
  ...
  const collection = requireFirestore(firestore).collection(firestoreCollections.runs);

  const query = projectId
    ? collection.where('projectId', '==', projectId).orderBy('createdAt', 'desc').limit(limit)
...
export async function listEvaluations(
  ...
  const collection = requireFirestore(firestore).collection(firestoreCollections.evaluations);
...
export async function getProjectMetrics(
  ...
  const projectDocument = await client.collection(firestoreCollections.projects).doc(projectId).get();
  ...
  const totalLatency = latestRuns.reduce((sum, run) => sum + (run.latencyMs ?? 0), 0);
  ...
  return {
    ...
    summary: {
      ...
      averageLatencyMs: latestRuns.length ? Math.round(totalLatency / latestRuns.length) : 0,
...
export async function getObservabilityOverview(
  ...
  const totalConfidence = runs.reduce((sum, run) => sum + (run.confidence ?? 0), 0);
  const totalEstimatedCost = runs.reduce((sum, run) => sum + (run.estimatedCostUsd ?? 0), 0);
  ...
  const projectBreakdown = projects.map((project) => {
    ...
    const successfulRuns = projectRuns.filter(
      (run) => run.evaluationStatus === 'passed' && !run.fallbackTriggered && !run.escalated,
    );
    const totalProjectLatency = projectRuns.reduce((sum, run) => sum + (run.latencyMs ?? 0), 0);
    const totalProjectConfidence = projectRuns.reduce((sum, run) => sum + (run.confidence ?? 0), 0);
    ...
    return {
      ...
      passRate: projectRuns.length
        ? Number(((successfulRuns.length / projectRuns.length) * 100).toFixed(1))
        : 0,
  ...
  return {
    ...
    summary: {
      ...
      escalatedRuns: runs.filter((run) => run.escalated || run.status === 'escalated').length,
      fallbackRate: runs.length
        ? Number(((runs.filter((run) => run.fallbackTriggered).length / runs.length) * 100).toFixed(1))
        : 0,
      passRate: runs.length ? Number(((passedRuns / runs.length) * 100).toFixed(1)) : 0,
      averageLatencyMs: runs.length ? Math.round(totalLatency / runs.length) : 0,
      averageConfidence: runs.length ? Number((totalConfidence / runs.length).toFixed(3)) : 0,
...
export async function persistDemoRun(
  ...
  batch.set(client.collection(firestoreCollections.evaluations).doc(payload.evaluation.id), {
    ...payload.evaluation,
    observedAt: seededAt,
  });

  for (const toolInvocation of payload.toolInvocations) {
    batch.set(client.collection(firestoreCollections.toolInvocations).doc(toolInvocation.id), {
      ...toolInvocation,
      observedAt: seededAt,
    });
  }
  ...
  if (payload.escalation) {
    batch.set(client.collection(firestoreCollections.escalations).doc(payload.escalation.id), {
      ...payload.escalation,
      observedAt: seededAt,
    });
  }
