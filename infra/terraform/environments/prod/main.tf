locals {
  ...
  github_pool_id     = "github-actions-prod"
  github_provider_id = "github-prod"
  ...
  dashboard_json = jsonencode({
    displayName      = "portfolio_tq prod overview"
    dashboardFilters = []
    mosaicLayout = {
      columns = 48
      tiles = [
...
module "deploy_service_account" {
  ...
  project_id               = var.project_id
  account_id               = "github-deploy-prod"
  display_name             = "GitHub deploy account for prod"
...
module "runtime_service_account" {
  ...
  project_id   = var.project_id
  account_id   = "portfolio-api-runtime-prod"
  display_name = "portfolio_tq API runtime account for prod"
...
module "artifact_registry" {
  ...
  description   = "Container images for the prod API service."
